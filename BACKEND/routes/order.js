  ...
  try {
    ...
    if (orderChannel === "walk-in") {
      if (!req.user.role || req.user.role !== "distributor") {
        return res
          .status(403)
          .send("Walk-in orders can only be created by distributors");
      }
    }
      ...
      query.orderChannel = channel;
		...
		if (status) {
			...
			if (invalid.length > 0) {
				return res.status(400).json({
					success: false,
					message: `Invalid status value(s): ${invalid.join(", ")}. Allowed values: ${ALLOWED_STATUSES.join(", ")}`,
				});
			}
			query["paymentInfo.deliveryStatus"] = requested.length === 1 ? requested[0] : { $in: requested };
		}
